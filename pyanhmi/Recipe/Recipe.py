...
class Recipe:
    ...
    def updated(cls, recipe_1: "Recipe", recipe_2: "Recipe"):
        ...
        def create_actions(action_funcs: list, attr_name: str = None):
            # actions = {}
            # for action_func in action_funcs:
            #     action = Action(action_func, based_on_cls)
            #     actions[action.__hash__] = action
            # return actions
            return [Action(action_func, new_recipe.based_on_cls, attr_name) for action_func in action_funcs]
        ...
        # for field_name in set(list(recipe_1.ingredients.keys()) + list(recipe_2.ingredients.keys())):
            # do not use set(list(recipe_1.ingredients.keys()) + list(recipe_2.ingredients.keys())) to manage the order of fields
            if new_recipe.ingredients.get(field_name):
                continue
            ...
            if field_name not in recipe_2.ingredients:
                ...
                continue
            # print(f"---------- 1 field_name: {field_name}")
            new_recipe.ingredients[field_name] = Field.updated(Field,
                                                               recipe_1.ingredients[field_name],
                                                               recipe_2.ingredients[field_name])
            # print(f"---------- 2 field_name: {field_name}")
            new_recipe.ingredients[field_name].pre_actions = create_actions(new_recipe.ingredients[field_name].pre_action_funcs, field_name)
            # print(f"---------- 3 field_name: {field_name}")
            new_recipe.ingredients[field_name].post_actions = create_actions(new_recipe.ingredients[field_name].post_action_funcs, field_name)
        return new_recipe
