...
class ObjectsNormalizer:
    ...
    def add(self, obj):
        if hasattr(type(obj), Config.normalize_rules_field_name_2) :
    ...
    def get_latest_objs(self, *args):
        ...
        """
        if len(args) == 0:
            return tuple([ObjectsNormalizer.get_real_obj(self.sources[obj_type][-1]) for obj_type in self.sources.keys()])
        elif len(args) == 1:
            return ObjectsNormalizer.get_real_obj(self.sources[args[0]][-1])
        else:
            return tuple([ObjectsNormalizer.get_real_obj(self.sources[obj_type][-1]) for obj_type in args])

    @staticmethod
    def _get_obj_by_idx_in_slice(sources_slice: list, obj_idx: int) -> Tuple[int, Any]:
        for cls_idx in range(0, len(sources_slice)):
            if ObjectsNormalizer.get_obj_idx(sources_slice[cls_idx]) == obj_idx:
    ...
    def _get_latest_obj_in_slice(sources_slice: list) -> Tuple[int, Any]:
        ...
        for idx in range(1, len(sources_slice)):
            if ObjectsNormalizer.get_obj_idx(sources_slice[idx]) > ObjectsNormalizer.get_obj_idx(latest_obj):
    ...
    def _get_oldest_obj_in_slice(sources_slice: list) -> Tuple[int, Any]:
        ...
        for cls_idx in range(1, len(sources_slice)):
            if ObjectsNormalizer.get_obj_idx(sources_slice[cls_idx]) < ObjectsNormalizer.get_obj_idx(oldest_obj):
    ...
    def get_all_objs(self):
        ...
        for obj_idx in range(self.obj_count):
            ...
            result.append(ObjectsNormalizer.get_real_obj(oldest_obj))
    ...
    def export(self, target_normalize_fields=None):
        ...
        latest_sources.sort(key=lambda s: ObjectsNormalizer.get_obj_idx(s))
        result = {}
